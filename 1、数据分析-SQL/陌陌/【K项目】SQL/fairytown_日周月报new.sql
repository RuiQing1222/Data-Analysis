...
case when country ='US'  then '美国'
     ...
     when country ='TR'  then '土耳其'
     else 'others'
end as country,
case when country in ('US','DE','FR','GB','AU','CA','NZ','SE','DK','NO','FI','IT','ES','RU','NL','BE','PL','AT','CH','TH','BR','MY','JP','KR','ID','TR') then country
     else 'others'
end as country2
...
union all
select to_date(cast (date_time as timestamp)) as birth_dt,channel_id,device_id,
case when country in ('HK','TW','MO') then '港澳台' 
     else 'others'
end as country,
...
新增用户&新增付费用户&新增收入
select  birth_dt,a.channel_id,country,country2,media_source,
...
sum(case when datediff(pay_dt,birth_dt) <=149 then pay_price else 0 end) as pay_day150,
sum(case when datediff(pay_dt,birth_dt) <=179 then pay_price else 0 end) as pay_day180,
sum(case when datediff(pay_dt,birth_dt) <=209 then pay_price else 0 end) as pay_day210,
sum(case when datediff(pay_dt,birth_dt) <=239 then pay_price else 0 end) as pay_day240,
sum(case when datediff(pay_dt,birth_dt) <=269 then pay_price else 0 end) as pay_day270,
sum(case when datediff(pay_dt,birth_dt) <=299 then pay_price else 0 end) as pay_day300,
sum(case when datediff(pay_dt,birth_dt) <=329 then pay_price else 0 end) as pay_day330,
sum(case when datediff(pay_dt,birth_dt) <=359 then pay_price else 0 end) as pay_day360
-- sum(pay_price) as 'pay_all'
from
(select birth_dt,channel_id,device_id,country,country2,
case when media_source  is not null then media_source
     when media_source  is null     then 'Organic'
...
新增留存
select birth_dt,a.channel_id,media_source,
case when campaign_type is not null then campaign_type
     when campaign_type is null then 'Organic'
end as install_source ,
case when campaign_type is not null then 'Paid'
...
--      else 'others'
-- end as country,
-- case when country  in ('US','DE','FR','GB','AU','CA','NZ','SE','DK','NO','FI','IT','ES','RU','NL','BE','PL','AT','CH')    then country
--      else 'others'
-- end as country2
from fairy_town.device_activate
where day_time>=20210916 and day_time< ${endDate}
...
where datediff(login_dt,birth_dt) in (0,1,2,3,4,5,6)
group by 1,2,3,4,5,6,7
