...
when '95' then '限时建筑商店'
when '96' then '商城广告'
when '97' then '定时家园宝箱广告'
when '98' then '加速可持续生产建筑'
when '99' then '好友送礼'
when '100' then '邀请累计系统'
...
round(sum(get_count),2) as '炸弹获取数量', 
round(sum(get_count)/count(distinct user_id),2) as '账户平均获得炸弹数量',
sum(case when consume_currency_id = '2' then cast(consume_currency_count as int) else 0 end) as '消耗钻石数量', 
round(sum(case when consume_currency_id = '2' then cast(consume_currency_count as int) else 0 end)/count(distinct user_id),2) as '账户平均消耗钻石数量',
round(avg(case when consume_currency_id = '2' then cast(consume_currency_count as int) else 0 end),2) as '平均每次消耗钻石数量',
count(distinct user_id) as '账户数'
from fairy_town_server.server_bomb_get
where 
day_time between ${beginDate} and ${endDate}
      and server_id in (${serverIds})
group by get_method, bomb_id
order by 炸弹获取数量 desc;
