...
consume_currency_id as '消耗货币ID',
case consume_currency_id
when '1' then '金币'
when '2' then '钻石'
when '3' then '体力'
end as '消耗货币类型', 
sum(cast(get_props_count as int)) as '购买道具数量', 
round(sum(cast(get_props_count as int))/count(distinct user_id),2) as '账户平均购买道具数量',
sum(consume_currency_count) as '消耗货币数量',
round(sum(consume_currency_count)/count(distinct user_id),2) as '账户平均消耗货币数量',
round(sum(consume_currency_count)/sum(cast(get_props_count as int)),2) as '道具平均消耗货币数量',
count(distinct user_id) as '购买该道具账户数'
from fairy_town_server.server_market_buy
where 
day_time between ${beginDate} and ${endDate}
      and server_id in (${serverIds})
group by get_props_id, consume_currency_id
order by get_props_id desc;