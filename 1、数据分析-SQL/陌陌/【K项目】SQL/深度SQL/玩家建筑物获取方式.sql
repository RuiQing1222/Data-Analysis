...
get_method as '获取建筑物方式ID',
case get_method 
when '10' then '云团解锁'
when '11' then '购买建筑' 
end as '获取建筑物方式',
count(distinct user_id) as '账户数',
round(count(user_id)/count(distinct user_id),2) as '每个账户获得建筑物数量', 
...
group by building_id, get_method
order by 获取建筑物ID,获取建筑物方式;