...
complete_stage as '建筑所处阶段', 
queue_num as '扩展栏位(第~个)', 
sum(queue_price) as '扩展栏位花费',
count(distinct user_id) as '扩展栏位账户数'
FROM fairy_town_server.server_buy_industry1_queue_num 
where 
day_time between ${beginDate} and ${endDate}
      and server_id in (${serverIds})
group by building_type, building_id, complete_stage, queue_num
