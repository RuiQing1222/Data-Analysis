-- 第三期
-- 1.20  12：00 -  2.15 3：00 
战令数据

1642651200000   -   1644865200000
...
select
    ...
    case when birth_dt >= '2022-01-20' and birth_dt <= '2022-02-15' then '活动内'
         else '活动之前'
         end as '是否活动期内',
     最后登录日期,
    case when 最后登录日期 < '2022-02-01' then '流失'
         else '留存'
         end as '是否流失', 
...
where day_time>=20220120 and day_time<=${endDate} 
and server_id in (10001,10002,10003)
and product_name='战令'
) a 
left join 
