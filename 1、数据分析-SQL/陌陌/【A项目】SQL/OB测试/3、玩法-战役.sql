20001,20002,20003,20004,20005,20006,20007,20008,20009,20010,20011,20012,20013,20014,20015,20016
...
count(distinct case when battle_result = 2 and remain_hp = 0 and battle_time <> '1超时' then a.role_id else null end) as '死亡人数',
count(distinct case when battle_result = 2 and battle_time = '1超时' and remain_hp <> 0 then a.role_id else null end) as '超时人数',
...
-- (select a.role_id,enter_dt,dungeon_id,num--,die_or_not
--  from
...
right join
(select device_id,to_date(cast(date_time as timestamp)) as device_birth_dt,country
...
-- where datediff(done_dt,birth_dt)+ 1 <= ${left_time}
-- group by 1 order by 1
...
-- where datediff(f.done_dt,a.birth_dt)+ 1 <= ${left_time}
-- group by 1
-- order by 1
...
-- from myth_server.server_enter_dungeon
-- where day_time between ${beginDate} and ${endDate} and server_id in (${serverIds})
-- and game_type in (2,4,6,7,8,9,10,11,12,13,14,16,17,18,19,20,21,22)
-- and version_name in ('1.5.1','1.5.2')
-- and country not in ('CN','HK')
-- ) as d
...
left join
(select dungeon_id,role_id,battle_points,to_date(cast(date_time as timestamp)) as done_dt
from myth_server.server_enter_dungeon
where day_time between ${beginDate} and ${endDate} and server_id in (${serverIds})
and game_type = 3 
and channel_id in (1000,2000) 
...
on a.role_id = b.role_id
where datediff(b.done_dt,a.birth_dt) <= ${lifeTime}-1
) as new
group by 1,2
order by 1,2
...
from 
(
select role_id,to_date(date_time) as end_dt,dungeon_id,start_time 
from myth_server.server_enter_dungeon
...
left join 
(
select role_id,to_date(date_time) as end_dt,battle_result,dungeon_id,start_time
from myth_server.server_dungeon_end
where day_time>=${beginDate} and day_time<=${end2Date}
and version_name in ('1.5.1','1.5.2')
and country not in ('CN','HK')
...
-- from 
-- (select dungeon_id,role_id,role_type,start_time,to_date(cast(date_time as timestamp)) as done_dt
-- from myth_server.server_enter_dungeon
-- where day_time between ${beginDate} and ${endDate} and server_id in (${serverIds})
-- and game_type = 3 -- 3->战役
-- and channel_id in (1000,2000) 
...
-- left join
-- (
-- select dungeon_id,role_id,role_type,start_time,end_time,标签--,是否死亡
-- from
-- (
-- select a.role_id,role_type,a.start_time,a.dungeon_id,end_time,1 as '标签',是否死亡
-- from
-- (select dungeon_id,role_id,role_type,to_date(cast(date_time as timestamp)) as done_dt,start_time
-- from myth_server.server_enter_dungeon
-- where day_time>=${beginDate} and day_time<=${endDate} and server_id in (${serverIds})
-- and game_type =3 -- 3战役
-- and channel_id in (1000,2000) 
...
-- select a.role_id,role_type,a.start_time,a.dungeon_id,end_time,2 as '标签',0 as '是否死亡'
-- from
-- (select dungeon_id,role_id,role_type,to_date(cast(date_time as timestamp)) as done_dt,start_time
-- from myth_server.server_enter_dungeon
-- where day_time>=${beginDate} and day_time<=${endDate} and server_id in (${serverIds})
...
-- left join
-- (
-- select dungeon_id,role_id,role_type,start_time,end_time,标签
-- from
-- (
-- select a.role_id,role_type,a.start_time,a.dungeon_id,end_time,1 as '标签'
-- from
-- (select dungeon_id,role_id,role_type,to_date(cast(date_time as timestamp)) as done_dt,start_time
-- from myth_server.server_enter_dungeon
-- where day_time>=${beginDate} and day_time<=${endDate} and server_id in (${serverIds})
...
-- select a.role_id,role_type,a.start_time,a.dungeon_id,end_time,2 as '标签'
-- from
-- (select dungeon_id,role_id,role_type,to_date(cast(date_time as timestamp)) as done_dt,start_time
-- from myth_server.server_enter_dungeon
-- where day_time>=${beginDate} and day_time<=${endDate} and server_id in (${serverIds})
...
select dungeon_id,
                  ...
                  case when god_card_id in (10101,10105,10106,10107,10108,10109,10110,10111,10112,10113,10114,10115,10116) then '1-奥丁-主神卡'
                       when god_card_id in (20101,20105,20106,20107,20108,20109,20110,20111,20112,20113,20114,20115,20116) then '2-希芙-主神卡'
                       when god_card_id in (30101,30105,30106,30107,30108,30109,30110,30111,30112,30113,30114,30115,30116) then '3-弗丽嘉-主神卡'
                       when god_card_id in (40101,40105,40106,40107,40108,40109,40110,40111,40112,40113,40114,40115,40116) then '4-海姆达尔-主神卡'
                       when god_card_id in (50101,50105,50106,50107,50108,50109,50110,50111,50112,50113,50114,50115,50116) then '5-巴德尔-主神卡'
                       when god_card_id in (60101,60105,60106,60107,60108,60109,60110,60111,60112,60113,60114,60115,60116) then '6-瓦尔基里-主神卡'
                  else null
...
left join 
-- 时长 通关-死亡-主动退出remain_hp 是否为0
(select dungeon_id,role_id,role_type,god_card_id,start_time,battle_result,battle_time,auto_battle,done_dt,cast(split_part(remain_hp,',',1) as int) as remain_hp,suppression
from
(select a.dungeon_id,a.role_id,role_type,god_card_id,a.start_time,battle_result,battle_time,auto_battle,done_dt,remain_hp,suppression
from 
(select dungeon_id,role_id,role_type,god_card_id,start_time,to_date(cast(date_time as timestamp)) as done_dt
from myth_server.server_enter_dungeon
where day_time between ${beginDate} and ${endDate} and server_id in (${serverIds})
and game_type = 3 
and channel_id in (1000,2000) 
...
left join
(select dungeon_id,role_id,role_type,battle_points,to_date(cast(date_time as timestamp)) as done_dt
from myth_server.server_enter_dungeon
where day_time between ${beginDate} and ${endDate} and server_id in (${serverIds})
and game_type = 3 
and channel_id in (1000,2000) 
...
left join 
(
select role_id,to_date(date_time) as done_dt,role_type,max(dungeon_id) as max_dungeon
from myth_server.server_dungeon_end
where day_time>=${beginDate} and day_time<=${endDate}
and version_name in ('1.5.1','1.5.2')
and country not in ('CN','HK')
...
-- from 
-- (select dungeon_id,role_id,god_card_id,start_time,to_date(cast(date_time as timestamp)) as done_dt
-- from myth_server.server_enter_dungeon
-- where day_time between ${beginDate} and ${endDate} and server_id in (${serverIds})
-- and game_type = 3 -- 3->战役
-- and channel_id in (1000,2000) 
...
-- left join
-- (
-- select dungeon_id,role_id,god_card_id,start_time,end_time,标签--,是否死亡
-- from
-- (
-- select a.role_id,god_card_id,a.start_time,a.dungeon_id,end_time,1 as '标签',是否死亡
-- from
-- (select dungeon_id,role_id,god_card_id,to_date(cast(date_time as timestamp)) as done_dt,start_time
-- from myth_server.server_enter_dungeon
-- where day_time>=${beginDate} and day_time<=${endDate} and server_id in (${serverIds})
-- and game_type =3 -- 3战役
-- and channel_id in (1000,2000) 
...
-- select a.role_id,god_card_id,a.start_time,a.dungeon_id,end_time,2 as '标签',0 as '是否死亡'
-- from
-- (select dungeon_id,role_id,god_card_id,to_date(cast(date_time as timestamp)) as done_dt,start_time
-- from myth_server.server_enter_dungeon
-- where day_time>=${beginDate} and day_time<=${endDate} and server_id in (${serverIds})
...
left join
(
select dungeon_id,role_id,god_card_id,start_time,end_time,标签
from
(
select a.role_id,god_card_id,a.start_time,a.dungeon_id,end_time,1 as '标签'
from
(select dungeon_id,role_id,god_card_id,to_date(cast(date_time as timestamp)) as done_dt,start_time
from myth_server.server_enter_dungeon
where day_time>=${beginDate} and day_time<=${endDate} and server_id in (${serverIds})
...
select a.role_id,god_card_id,a.start_time,a.dungeon_id,end_time,2 as '标签'
from
(select dungeon_id,role_id,god_card_id,to_date(cast(date_time as timestamp)) as done_dt,start_time
from myth_server.server_enter_dungeon
where day_time>=${beginDate} and day_time<=${endDate} and server_id in (${serverIds})
...
left join
(select dungeon_id,role_id,god_card_id,battle_points,to_date(cast(date_time as timestamp)) as done_dt
from myth_server.server_enter_dungeon
where day_time between ${beginDate} and ${endDate} and server_id in (${serverIds})
and game_type = 3 
and channel_id in (1000,2000) 
...
from 
(
select role_id,to_date(date_time) as end_dt,dungeon_id,start_time,god_card_id 
from myth_server.server_enter_dungeon
where day_time>=${beginDate} and day_time<=${end2Date}
and version_name in ('1.5.1','1.5.2')
and country not in ('CN','HK')
