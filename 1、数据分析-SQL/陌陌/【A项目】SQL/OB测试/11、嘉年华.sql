...
select birth_dt,vip,datediff(done_dt,birth_dt)+1 as '生命周期',max_dungeon,count(distinct a.role_id)
from 
(select birth_dt,role_id,vip,login_times
from 
...
case when total_pay>0    and total_pay<=8    then 1
     ...
     count(distinct login_dt) as login_times    
from 
(select birth_dt,a.role_id
...
where datediff(login_dt,birth_dt)<10   --10日内
group by 1,2,3
) a 
where login_times>=7
) a    
...
on a.role_id = e.role_id
where datediff(done_dt,birth_dt)<10
group by 1,2,3,4
