...
整体钻石消耗
select birth_dt,b.role_id as role_id,boss_id,datediff(consume_dt,birth_dt)+1 as '天数',max_damage_value,最高伤害分层,change_count
from
...
left join
(select b.role_id,boss_id,max_damage_value,最高伤害分层,change_count,consume_dt
from
...
left join
(SELECT role_id,to_date(cast(date_time as timestamp)) as consume_dt,sum(change_count) as change_count 
