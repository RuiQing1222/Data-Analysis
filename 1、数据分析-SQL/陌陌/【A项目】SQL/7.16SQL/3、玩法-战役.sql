...
select suppression,a.role_id,server_id
from

(  --新增
select role_id,birth_dt,country,server_id
from
(select role_id,device_id,to_date(cast(date_time as timestamp)) as birth_dt,server_id
from myth.server_role_create 
where day_time>=${beginDate} and day_time<=${birthEndDate} 
and server_id in (${serverIds}) 
and version_name ='1.5.0'
...
on a1.device_id = a2.device_id 
 ...
 -- 3-13 绿色和白色没有通过的玩家   没有后续闯关的玩家
left join
(select c.role_id,suppression,done_dt
from 
(select role_id,
        case when suppression = 84 then '绿色'
             when suppression >= 85 and suppression <= 95 then '白色'
...
from
(select role_id,to_date(cast(date_time as timestamp)) as done_dt,max(dungeon_id) as m_dungeonid
from myth_server.server_enter_dungeon
where day_time between ${beginDate} and ${endDate} and server_id in (${serverIds})
and game_type = 3 -- 3->战役
and channel_id=1000  --Android
