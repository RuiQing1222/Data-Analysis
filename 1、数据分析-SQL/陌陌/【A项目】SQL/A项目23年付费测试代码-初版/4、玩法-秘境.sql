...
整体、付费-- 人数维度进入关卡人数、通关率、无日志率，D1次数维度数据
...
整体、付费 -- 复玩率
select birth_dt,dungeon_id,datediff(done_dt,birth_dt)+1 as '天数',标签,count(distinct b.role_id) as '进入总人数',count(distinct case when 进入次数 > 1 then b.role_id else null) as '复玩人数'
from
...
(select dungeon_id,role_id,done_dt,count(distinct start_time) as '进入次数'
from
(select dungeon_id,role_id,to_date(cast(date_time as timestamp)) as done_dt,start_time -- start_time也可用统计参与次数
from myth_server.server_enter_dungeon
where day_time between ${beginDate} and ${endDate} and server_id in (${serverIds})
and game_type = 2 -- 2->秘境
and channel_id=1000  --Android
...
group by 1,2,3,4
union all
select dungeon_id,role_id,to_date(cast(date_time as timestamp)) as done_dt,log_time as start_time -- log_time也可用统计参与次数
...
扫荡数据 2->秘境 整体 - 付费
扫荡单独算
select birth_dt,b.role_id as role_id,date_time,dungeon_id,标签,datediff(enter_dt,birth_dt)+1 as '天数'
from
...
on a.role_id = c.role_id and done_dt = pay_dt
...
select birth_dt,datediff(done_dt,birth_dt)+1 as '天数',标签,count(distinct start_time)
from
...
left join
(select role_id,start_time,to_date(cast(date_time as timestamp)) as done_dt
from myth_server.server_dungeon_end
...
on a.role_id = c.role_id and b.done_dt = c.pay_dt
where datediff(done_dt,birth_dt) in (0,1,2,3,4,5,6,13,29)
group by 1,2,3
order by 1,2
...
免费券消耗、付费券消耗
select birth_dt,datediff(consume_dt,birth_dt)+1 as '天数',免费券消耗次数,付费券消耗次数,标签
from
(  --新增
select role_id,birth_dt
from
...
left join
(select role_id,consume_dt,sum(case when 有无付费券 = '有' then 3 else num end) as '免费券消耗次数',
                          sum(case when 有无付费券 = '有' then (num-3) else 0 end) as '付费券消耗次数'
from
(select role_id,consume_dt,case when num <= 3 then '无' else '有' end as '有无付费券' 
from
(select role_id,to_date(cast(date_time as timestamp)) as consume_dt,sum(change_count) as num
...
and prop_id = '00000' -- 入场券ID，要找策划要表
group by 1,2) b1
group by 1,2,3) b2
group by 1,2
) as b
...
select birth_dt,datediff(consume_dt,birth_dt)+1 as '天数',钻石购买次数,标签
from
(  --新增
select role_id,birth_dt
from
...
left join
(select role_id,to_date(cast(date_time as timestamp)) as consume_dt,count(distinct log_time) as '钻石购买次数'
